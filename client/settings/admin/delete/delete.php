					...
					<div class="col-12 col-md-9 col-lg-7 col-xl-6">
						<div class="card bg-dark" style="border-radius: 15px; color:  rgb(255, 223, 79); --bs-bg-opacity: 0.8;">
							<div class="card-body p-5">
								...
								<div>
									<a href="../delete.php">Choose Another Category</a>
								</div>
							</div>
						</div>
					</div>
		...
		<div>
			...
			<ul>
				...
				if (isset($_POST['delete'])) {
					...
					$set = 'SET @num = 0';
					$reset = 'UPDATE ' . $table . ' SET id = @num := (@num+1)';
					$auto = 'ALTER TABLE ' . $table . ' AUTO_INCREMENT = 1';
				...
				// Displays the Words in database, you know what to delete
				if ($table == 'phrases') {
					if ($result->num_rows > 0) {
						// output data of each row
						while ($row = $result->fetch_assoc()) {
							echo "<li>Word Id: " . $row["id"] . " - Mi'kmaw Word: " . $row["phrase"] . " - Response: " . $row["response"] . " - Phrase_Translation: " . $row["translation"] . " - Response_Translation: " . $row["respTranslation"] . "<br> - Phrase_Sound: " . $row["phraseSound"] . " - Response_Sound: " . $row["responseSound"] . "</li>";
						}
					} else {
						echo "0 results";
					}
					...
					if ($result->num_rows > 0) {
						// output data of each row
						while ($row = $result->fetch_assoc()) {
							echo "<li>Word Id: " . $row["id"] . " - Mi'kmaw Word: " . $row["word"] . " - Word_Translation: " . $row["englishWord"] . "<br> - Sound: " . $row["sound"] . "</li>";
						}
					} else {
						echo "0 results";
					}
					...
					if ($result->num_rows > 0) {
						// output data of each row
						while ($row = $result->fetch_assoc()) {
							echo "<li>Word Id: " . $row["id"] . " - Mi'kmaw Word: " . $row["word"] . " - Picture: " . $row["picture"] . " - Word_Translation: " . $row["englishWord"] . "<br> - Sound: " . $row["sound"] . "</li>";
						}
					} else {
						echo "0 results";
					}
