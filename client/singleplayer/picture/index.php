...
<body id="home">
			...
			<div>
				...
				<div class="picGameBtns">
					...
					<div class="row">
						...
						<button id="hint" style="margin: 20px;" class="btn btn-warning"><i class="fas fa-volume-up"></i> HINT </button>
					</div>
				</div>
				...
				$sql = "SELECT id, word, picture, sound, englishWord FROM $table WHERE id=$num";
				$result = $conn->query($sql);

				if ($result->num_rows > 0) {
					// output data
					while ($row = $result->fetch_assoc()) {
						echo "<div><img alt='picture' id='image' max-width='300' src='../../assets/game/back/{$row['picture']}'></div>";
						$sound = $row["sound"];
						$word = $row["word"];
				...
				<div class="solver">
					<form>
						...
						<div class="keyboard-section" style="font-family: serif;">
							...
							<div id="-" class="keyboard-letters temp-disabled">-</div>
						</div>
					</form>
				</div>
			</div>
	...
	<div>
		<audio id="sounds">
			<source src='../../assets/game/back/<?php echo $sound ?>' type="audio/mpeg">
		</audio>
		<audio id="letter">
	...
	<form name="winScore" action="" method="post">
		<input type="hidden" id="usrScore2" name="usrScore" value="" />
	...
	<script>
		...
		var translation = "<?php echo $englishWord ?>";
	...
	if (isset($_POST['solveBtn'])) {
		...
		if (((strtoupper($guess)) == (strtoupper($word)))) {
			...
			<script type='text/javascript'>
					...
					winner.innerHTML = '<span id=\"close\" class=\"close\">&times;</span><h3>Congratulations</h3><div style=\"font-family: serif;\"><h2> The Correct Word: " . $word . "</h2><h2> The Word Translation: " . $englishWord . "</h2></div>';
			...
			<script type='text/javascript'>
				...
				var winner = document.getElementById('winner'); 
				win.style.display = 'block';
				winner.innerHTML = '<span id=\"close\" class=\"close\">&times;</span><h3>Incorrect</h3><div style=\"font-family: serif;\"><h2> The Correct Word: " . $word . "</h2><h2> The Word Translation: " . $englishWord . "</h2></div>';
				var span1 = document.getElementById('close');
				span1.onclick = function () {
					win.style.display = \"none\";
					var resetWin = document.getElementById(\"shuffling\");
